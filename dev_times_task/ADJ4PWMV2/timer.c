...
extern PWMInfoDef pwm2Info;	//中间变量，用户请勿修改。
...
void timer0_int (void) interrupt TIMER0_VECTOR
{
	if(P_PWM1_P35)
	{
		TH0 =  (u8)(pwm1Info.pwm_low>> 8);	//如果是输出高电平，则装载低电平时间。
		TL0 = (u8)pwm1Info.pwm_low;
	}
	else
	{
		TH0 = (u8)(pwm1Info.pwm_high >> 8);	//如果是输出低电平，则装载高电平时间。
...
void timer2_int (void) interrupt TIMER2_VECTOR
{
	if(P_PWM2_p30)
	{
		TH2 =  (u8)(pwm2Info.pwm_low>> 8);	//如果是输出高电平，则装载低电平时间。
		TL2 = (u8)pwm2Info.pwm_low;
	}
	else
	{
		TH2 = (u8)(pwm2Info.pwm_high >> 8);	//如果是输出低电平，则装载高电平时间。
