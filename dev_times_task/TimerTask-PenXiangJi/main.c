...
sbit ioOutForL1Green= P3^1;  //ÂÌµÆ£¬³ÌĞòÔËĞĞÖ¸Ê¾µÆ
sbit ioOutForL2Red  = P3^1;  //ºìµÆ£¬°´¼üÏìÓ¦Ö¸Ê¾µÆ
sbit ioOutForMotoPower=P3^4;  //ÅçÏã»úµçÔ´¿ª¹ØIO¿Ú. 
sbit ioOutForMotoKey= P3^0;  //ÅçÏã»úÇı¶¯¿ª¹ØIO¿Ú. 

u8 ioSwitchLED=0;
u8 ioWorkLED=0;
u8 ioKEY=0;

#define  k_moto_run_time 10; //50 //¶¨Ê±µş¼ÓÊı
u8 g_moto_run_time=0; //µç»úÆô¶¯ÔËĞĞºóµÄÊ±¼äÖµ
...
void	EXTI_config(void)
{
    ...
    //³õÊ¼»¯INT0
	EXTI_InitStructure.EXTI_Mode      = EXT_MODE_RiseFall;	//ÖĞ¶ÏÄ£Ê½,  EXT_MODE_RiseFall, EXT_MODE_Fall
...
void	GPIO_config(void)
{
	...
	GPIO_InitStructure.Pin  = GPIO_Pin_0 | GPIO_Pin_1| GPIO_Pin_5;	//Ö¸¶¨Òª³õÊ¼»¯µÄIO, 
	...
	GPIO_InitStructure.Pin   = GPIO_Pin_2 |GPIO_Pin_3 | GPIO_Pin_4 ;	//Ö¸¶¨Òª³õÊ¼»¯µÄIO,
...
void main(void)
{
    ...
    //È«¾Ö±äÁ¿³õÊ¹»¯
	// ¹¤×÷µÆ¿ªÊ¼
      ioOutForL1Green=ioInKeyForMoto=ioInKeyForRun=ioInKeyForTime=1; //key¼üÀ­¸ß
	...
	while (1)
	{
		...
		Ò»ÌìÅçÊ±¼äÉèÖÃÎª£º7µã£¬13µã£¬19µã
		*/
...
void secondAction(void) {
	...
	if(g_second%10 == 0){
        ...
        if(g_moto_run_time==0){
                ...
                g_moto_run_time--;
                //Æô¶¯µç»ú°´¼ü
                ioOutForMotoKey=0;  //´ò¿ª
               delay_ms(20);
               ioOutForMotoKey=1; //¹Ø±Õ
